...
function findBestHtml(dir) {
  ...
  for (const name of ["index.html", "index.htm", "home.html", "main.html", "default.html"]) {
...
async function ensureIndexHtml(projectDir) {
  ...
  await fs.copy(htmlFile, existingIndex, { overwrite: false });

  console.log(`Automatic entry created: ${htmlFile} -> ${existingIndex}`);
...
async function flattenSingleFolder(projectDir) {
  ...
  const visibleItems = items.filter(
    item => item.name !== "__MACOSX" && item.name !== ".DS_Store"
  );
  ...
  if (visibleItems.length === 1 && visibleItems[0].isDirectory()) {
    ...
    console.log(`Detected project root folder: ${folderName}`);

    const tempFlatten = path.join(
      DATA_DIR,
      `flatten-${Date.now()}-${Math.random().toString(36).slice(2)}`
    );

    await fs.ensureDir(tempFlatten);

    const innerItems = await fs.readdir(folderPath);
    ...
    const flattenedItems = await fs.readdir(tempFlatten);
...
app.get("/", (req, res) => {
  ...
  const totalSize = projects.reduce(
    (sum, project) => sum + Number(project.size || 0),
    0
  );
  ...
  const projectHTML = projects.length
      ...
      <div class="project" data-search="${escapeAttribute(
        ...
        <div class="project-info">
          ...
          <div class="project-url-row">
            <a
              class="project-url"
              href="/site/${encodeURIComponent(project.slug)}/"
              target="_blank"
              rel="noopener"
            >/site/${escapeHtml(project.slug)}/</a>
            <button
              class="copy-url"
              type="button"
              data-url="/site/${encodeURIComponent(project.slug)}/"
              title="Copy URL"
            >📋 Copy</button>
          </div>
        ...
        <div class="actions">
          ...
          <a
            ...
            href="/edit/${encodeURIComponent(project.name)}"
          >✏️ Edit</a>

          <form
            action="/delete/${encodeURIComponent(project.name)}"
            method="POST"
    ...
    `).join("")
    : `<div class="empty"><div class="empty-icon">📂</div><b>এখনো কোনো project deploy করা হয়নি।</b><span>উপরের Deploy box থেকে প্রথম website তৈরি করুন।</span></div>`;
...
body::after{
  width:450px;height:450px;bottom:-180px;right:-180px;
...
.bg-light{
  ...
  background:var(--c1);box-shadow:0 0 16px var(--c1), 0 0 24px var(--c2);
...
.light5{left:20%;top:40%;animation-delay:3s}
.light6{left:75%;top:70%;animation-delay:5s}

@keyframes floatLight{
  0%{transform:translate3d(0,0,0) scale(.8);opacity:.2}
  50%{opacity:.9;box-shadow:0 0 22px var(--c3), 0 0 32px var(--c1)}
  100%{transform:translate3d(0,-260px,0) scale(1.3);opacity:0}
}
...
.deploy-btn{
  position:relative;width:100%;border:0;padding:17px;border-radius:13px;
  color:white;font-size:18px;font-weight:900;cursor:pointer;overflow:hidden;
...
.project:hover{
  ...
  box-shadow:0 0 25px rgba(0,217,255,.16);
}
...
.project-meta{
  ...
  color:#64748b;font-size:12px;margin-top:9px;
}

.project-meta span:last-child{color:#4ade80}

.actions{
  display:flex;gap:8px;position:relative;z-index:5;
...
.empty{
  text-align:center;padding:42px 20px;color:#64748b;
...
.toast{
  ...
  color:white;padding:12px 17px;border-radius:12px;
...
<div class="bg-light light5"></div>
<div class="bg-light light6"></div>
...
<div id="dropBox" class="drop">
  ...
  <div style="font-size:18px;font-weight:bold">
    HTML / CSS / JS অথবা ZIP ফাইল নির্বাচন করুন
  </div>
  <div style="margin:8px 0 14px;color:#64748b;font-size:13px">
    Drag & Drop করেও file দেওয়া যাবে
  </div>
  ...
  <input
    ...
    multiple
    required
    accept=".html,.htm,.css,.js,.json,.png,.jpg,.jpeg,.gif,.svg,.webp,.ico,.txt,.zip"
  >

  <div id="fileStatus" style="margin-top:10px;color:#64748b;font-size:13px">
    কোনো file নির্বাচন করা হয়নি
  </div>
</div>

<button id="deployBtn" class="deploy-btn" type="submit">
🚀 DEPLOY WEBSITE
</button>
</form>
</div>
...
const titles = [
  {
    text:"🌐 TM Website Deploy",
    colors:["#00d9ff","#0066ff","#8b5cf6"]
  },
  {
    text:"👨‍💻 Developer Tamim Khan",
    colors:["#8b5cf6","#0066ff","#00d9ff"]
  }
];
...
const themes = [
  {
    c1:"#00d9ff",c2:"#0066ff",c3:"#8b5cf6",
    bg1:"#020617",bg2:"#07132f"
  },
  {
    c1:"#8b5cf6",c2:"#2563eb",c3:"#00d9ff",
    bg1:"#050817",bg2:"#0b1635"
  },
  {
    c1:"#00c896",c2:"#00a8cc",c3:"#2563eb",
    bg1:"#02130f",bg2:"#061d29"
  },
  {
    c1:"#f59e0b",c2:"#ef4444",c3:"#8b5cf6",
    bg1:"#120b04",bg2:"#20101b"
  }
];
...
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><title>No HTML Found</title></head>
...
<style>
body{
margin:0;background:radial-gradient(circle at top,#172554,#020617);
color:white;font-family:Arial;display:flex;justify-content:center;align-items:center;min-height:100vh
}
.box{
width:min(600px,90%);background:rgba(15,23,42,.94);border:1px solid #334155;
border-radius:22px;padding:40px;text-align:center;box-shadow:0 0 50px rgba(56,189,248,.25)
}
.success{font-size:60px;animation:pulse 1.5s infinite}
@keyframes pulse{50%{transform:scale(1.15)}}
h1{color:#4ade80}
.url{
display:block;margin:25px 0;padding:15px;background:#020617;border:1px solid #38bdf8;
border-radius:10px;color:#38bdf8;word-break:break-all;text-decoration:none
}
.btn{display:inline-block;padding:13px 20px;background:#334155;color:white;border-radius:10px;text-decoration:none;margin:5px}
...
`);
      ...
      res.status(500).send(`
<h1 style="text-align:center;margin-top:100px;font-family:Arial;color:red">❌ Deployment Failed</h1>
<p style="text-align:center;font-family:Arial">File process করতে সমস্যা হয়েছে।</p>
<p style="text-align:center"><a href="/">← Dashboard</a></p>
`);
    }
  }
);
...
.container{width:min(850px,94%);margin:auto;position:relative;z-index:2}

.rgb-card{position:relative;margin-bottom:20px}
...
.card{
  background:rgba(8,15,35,.88);border:1px solid var(--border);border-radius:20px;
  padding:25px;margin-bottom:20px;box-shadow:0 20px 60px rgba(0,0,0,.35)
}
h1{font-size:clamp(26px,6vw,42px);margin-top:0;background:linear-gradient(90deg,#00d9ff,#0066ff,#8b5cf6);-webkit-background-clip:text;background-clip:text;color:transparent}
h2{color:#dbeafe}
label{display:block;font-weight:bold;margin:18px 0 8px}
input,textarea{width:100%;padding:15px;border:1px solid #334155;outline:none;border-radius:12px;background:#020617;color:white;font-size:16px}
textarea{min-height:110px;resize:vertical}
button,.btn{display:inline-block;border:0;padding:14px 20px;border-radius:12px;color:white;font-weight:800;font-size:16px;cursor:pointer;text-decoration:none;margin-top:20px}
...
.url-preview{padding:14px;border-radius:12px;background:#020617;border:1px solid #2563eb;color:#38bdf8;word-break:break-all;margin-top:10px}
...
<p class="help">
এখানে নতুন HTML / CSS / JS অথবা ZIP দিলে এই একই project-এর files replace হবে।
...
<h2>🔗 Custom URL</h2>
<p class="help">
এখানে URL-এর শেষের নাম পরিবর্তন করতে পারবে。<br>
যেমন: /site/tmclock/ → /site/tm-clock/
</p>
...
<input
  ...
  type="text"
  name="slug"
  value="${escapeAttribute(meta.slug || folderName)}"
  required
  pattern="[A-Za-z0-9_-]+"
  maxlength="50"
>
<div class="url-preview">
/site/<b id="slugPreview">${escapeHtml(meta.slug || folderName)}</b>/
</div>
...
<p class="help">
💡 SEO information Google search engine-কে website সম্পর্কে বুঝতে সাহায্য করবে।
Sitemap এবং robots.txt-ও automatically তৈরি হচ্ছে।
</p>
...
<a href="/edit/${encodeURIComponent(folderName)}">← Back</a>
</p>
`);
      }
      ...
      try {
        // ==================================================
        // NEW VERSION → SAME PROJECT DIRECTORY
        // ==================================================
...
font-family:Arial;color:white;background:radial-gradient(circle at top,#172554,#020617)
}
.box{
width:min(600px,90%);padding:40px;text-align:center;background:rgba(15,23,42,.95);
border:1px solid #334155;border-radius:22px;box-shadow:0 0 50px rgba(56,189,248,.2)
}
.ok{font-size:60px}
a{
display:inline-block;padding:14px 20px;margin:7px;border-radius:10px;background:#059669;
color:white;text-decoration:none;font-weight:bold
}
.back{background:#334155}
.url{
display:block;background:#020617;border:1px solid #38bdf8;color:#38bdf8;
padding:13px;border-radius:10px;word-break:break-all;text-decoration:none
}
</style>
</head>
<body>
...
<p>একই URL-এই নতুন version চলছে।</p>
<a class="url" href="${escapeAttribute(liveUrl)}" target="_blank">${escapeHtml(liveUrl)}</a>
...
`);
      ...
      let previousSlugs = Array.isArray(oldMeta.previousSlugs)
        ? oldMeta.previousSlugs
        : [];

      if (
        oldSlug !== newSlug &&
        !previousSlugs.includes(oldSlug)
      ) {
      ...
      setProjectMetadata(
        folderName,
        {
          slug: newSlug,
          title: String(
            req.body.title || oldMeta.title || folderName
          ).slice(0, 150),
          description: String(
            req.body.description || oldMeta.description || ""
          ).slice(0, 300),
          keywords: String(
            req.body.keywords || oldMeta.keywords || ""
          ).slice(0, 500),
          author: String(
            req.body.author || oldMeta.author || "Tamim Khan"
          ).slice(0, 100),
          previousSlugs
        }
      );
...
app.get(
  ...
  (req, res) => {
    ...
    try {
      ...
      urls = folders.map(folder => {
        const meta = metadata[folder.name] || {
          slug: folder.name
        };
  ...
  <url>
    <loc>${escapeHtml(
      `${base}/site/${encodeURIComponent(slug)}/`
    )}</loc>
...
app.use(
  ...
  async (req, res, next) => {
    try {
      const requestedSlug =
        sanitizeSiteName(req.params.sitename);
...
`);
      ...
      if (
        ...
        const newUrl =
          `/site/${encodeURIComponent(meta.slug)}${
            req.path === "/" ? "/" : req.path
          }`;
...
<h2>⚠️ No HTML file found</h2>
<p>এই project-এ কোনো HTML file নেই।</p>
</div>
`);
      }
